  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto mb-safe w-full max-w-3xl px-4 pb-3">
      <div className="mx-auto grid grid-cols-3 items-center rounded-2xl border border-black/5 bg-white/70 px-6 py-2.5 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/50">
        ...
        <Link href="/" aria-current={isActive("/") ? "page" : undefined} className="group col-start-2 -mb-1 flex items-center justify-center">
        ...
        {/* Settings */}
        <Link href="/settings" aria-current={isActive("/settings") ? "page" : undefined} className="group col-start-3 flex items-center justify-center text-neutral-700 hover:text-[color:var(--secondary)]">
